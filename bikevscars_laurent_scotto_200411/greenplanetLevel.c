...
#include "imageGreenplanetTitle.c"
#include "imageOval.c"
...
void showGreenplanetTitle(GameOption *gameoption)
{
    if((gameoption -> screennumber) == 2)
    {
        ...
        gluBuild2DMipmaps( GL_TEXTURE_2D, image_greenplanet_title.bytes_per_pixel,
                image_greenplanet_title.width, image_greenplanet_title.height,GL_RGBA, GL_UNSIGNED_BYTE,
                image_greenplanet_title.pixel_data );
        glEnable(GL_TEXTURE_2D);
        glColor3f(1.0, 1.0, 1.0);
        glLoadIdentity();
        glTranslatef(0, 0,-1.7);
        gluOrtho2D(0, 1280, 0, 720);
        glTranslatef(445, 665 ,0);
        glBegin(GL_QUADS);
        glTexCoord2f(0.0, 1.0);  glVertex2f(0, 0);
        glTexCoord2f(1.0, 1.0);  glVertex2f(245, 0);
        glTexCoord2f(1.0, 0.0);  glVertex2f(245, 21);
...
{    
    if((gameoption -> screennumber) == 2)
    {
        ...
        for (i = 0; i < biker -> greenplanet; i++)
        {
            ...
            glTranslatef(0, 0,-1.7);
            gluOrtho2D(0, 1280, 0, 720);
            glTranslatef(greenplanet -> x + 15 * i, greenplanet -> y ,0);
            glBegin(GL_QUADS);
            glTexCoord2f(0.0, 1.0);  glVertex2f(0, 0);
